...
LearnCpp: https://www.learncpp.com/cpp-tutorial/numeral-systems-decimal-binary-hexadecimal-and-octal/
Practice:
...
int main()
{
    const unsigned int status{ 172u };
    const unsigned int lowNibbleMask{ 15u };

    std::cout << "Status binary: " << std::bitset<8>{ status } << '\n';
    std::cout << "Low nibble mask hex: " << std::hex << lowNibbleMask << '\n';
    std::cout << "Status decimal: " << status << '\n';
    return 0;
}
